...
#include"header/kernel_Base.hpp"
		...
		if(node->isLeaf){
			...
			for(unsigned short k=0;k<8;++k){
				if(node->neighbor[k]!=NULL){
					...
					kernel_2D_P2P(node->N , node->location, node->neighbor[k]->N, node->neighbor[k]->location, tempK);
            ...
            //			Self potential
			kernel_2D_P2P(node->N , node->location, node->N , node->location, tempK);
...
void kernel_Base::kernel_2D_P2P(const unsigned long M, const vector<Point>& x, const unsigned long N, const vector<Point>& y, MatrixXd& kernel) {
	kernel	=	MatrixXd::Zero(M,N);
	for(unsigned long i=0;i<M;++i){
		if(x[i].target){
		for(unsigned long j=0;j<N;++j){
			if(x[i].panel != y[j].panel){
				kernel(i,j) =   kernel_Func(x[i],y[j]);
			}
		}
		}
