...
/*! Point in 2 dimesion */
struct Point {
	//unsigned long gindx; // gindx: global index respecto al arreglo vectorial "location"
	unsigned long panel; // En que panel dentro de la geometria se encuentra el punto
	double x;
	double y;
	bool target;
	Point() {};
	Point(unsigned long panel_, double x_, double y_, bool target_): panel(panel_), x(x_), y(y_), target(target_)   {}
